    ...
    DragulaService.prototype.handleModels = function (name, drake) {
        ...
        drake.on('drop', function (dropElm, target, source) {
            ...
            if (target === null) {
                ...
                _this.onDrop.emit(obj);
                return;
            }

            var idOfDropElm = dropElm.getAttribute('dnd-id');
            var indexParentOfTarget = target.getAttribute('index-parent');
            var valueOfTarget = target.getAttribute('value');

            var obj = {
                status: 'success',
                idOfDropElm: idOfDropElm,
                indexOfTarget: indexParentOfTarget,
                valueOfTarget: valueOfTarget
            };
            ...
            if (source.children.length > 0) {
                for (var i = 0; i < source.children.length; i++) {
                    if (source.children[i].attributes['dnd-id'].value === idOfDropElm) {
                        source.removeChild(source.children[i]); 
                    }
                }
            }

            var parentIndexOfTarget = target.getAttribute('index-parent');
            var valueOfTarget = target.getAttribute('value');

            if (target.children.length > 0) {
                for (var i = 0; i < target.children.length; i++) {
                    if (target.children[i].attributes['dnd-id'].value === idOfDropElm) {
                        target.children[i].setAttribute('index-parent', parentIndexOfTarget);
                        target.children[i].setAttribute('value', valueOfTarget);
                    }
                }
            }
